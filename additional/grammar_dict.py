...
def main():
    ...
    all grammatical possibilities (using symbols) of every inflection."""

    print("[bright_yellow]Pali grammar dictionary")
    ...
    with open(Path("./additional/grammar.css")) as f:
        grammar_css = f.read()

    header_templ = Template(filename=str(Path("./additional/header_grammar_dict.html")))

    html_header = str(header_templ.render(css=grammar_css))

    html_header += "<body><div class='pgrammar_dict'><table class='pgrammar_dict'>"
    ...
    for counter, i in enumerate(db):
        ...
        if i.stem == "-":
            data_line = (i.pali_1, i.pos, "indeclinable")
            html_line = f"<tr><td><b>{i.pos}</b></td><td>indeclinable</td><td>🔼({i.pali_1})</td><td colspan='2'>{i.meaning_1}</td></tr>"
            ...
            for row_number, row_data in enumerate(template_data):
                ...
                for column_number, cell_data in enumerate(row_data):
                    ...
                    if (row_number > 0 and
                        ...
                        # grammar = get_symbols(grammar).encode('ascii', 'xmlcharrefreplace').decode("utf-8")
                        ...
                        for inflection in cell_data:
                            if inflection:
                                ...
                                if inflected_word in all_words_set:
                                    ...
                                    html_line += f"<td>{gsymbol}({i.pali_clean})</td>"
                                    html_line += f"<td>{case_usage}{'(' + i.plus_case + ') ' if i.plus_case else ''}{i.meaning_1 if i.meaning_1 else i.meaning_2}</td>"
                                    html_line += f"<td>{summarize_construction(i)}</td>"
                                    html_line += "</tr>"
    ...
    # save pickle file
    with open(Path("./additional/grammar_dict_pickle"), "wb") as f:
    ...
    with open(Path("./additional/grammar_dict.tsv"), "w") as f:
...
def make_golden_dict(PTH, gd_data_list):
    ...
    zip_path = Path("./additional/pali-grammar.zip")

    ifo = ifo_from_opts({
        "bookname": "Pali Grammar",
        "author": "Bodhirasa (+Chris Tham)",
        "description": "Pali Grammar Dictionary with grammatical symbols, case usage, meaning and construction",
    ...
    if (
        ...
        popen(
            f'unzip -o {zip_path} -d "{goldendict_path}"')
    else:
...
def make_mdict(PTH, md_data_list):
    ...
    outfile = open(Path("./additional/dpd-mdict.mdx"), 'wb')
